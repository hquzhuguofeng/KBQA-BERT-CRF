...
def crf_convert_examples_to_features(examples,tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # input_ids [cls + 问题 + SEP]
        # token_type_ids [全零] 长度和input_ids一样长
        attention_mask = [1 if mask_padding_with_zero else 0] * len(input_ids)


        padding_length = max_length - len(input_ids) # 64 - 19
        input_ids = input_ids + ([pad_token] * padding_length) # [前面是cls+问题+sep+padding长度的零]
        attention_mask = attention_mask + ([0 if mask_padding_with_zero else 1] * padding_length) # [11110000] 1的部分是有内容的部分，0是padding的部分
        token_type_ids = token_type_ids + ([pad_token_segment_id] * padding_length) # [00000] 全零  长度128
    ...
    def _create_examples(cls, path):
        ...
        with codecs.open(path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                elif 1 == len(tokens) and '' == tokens[0]: # 遇到空格就append到list中
...
def load_and_cache_example(args,tokenizer,processor,data_type):
    ...
    cached_features_file = "cached_{}_{}".format(data_type, str(args.max_seq_length)) # cached_train_128
    cached_features_file = os.path.join(args.data_dir, cached_features_file) # ./input/data/ner_data\\cached_train_128
    if os.path.exists(cached_features_file):
        features = torch.load(cached_features_file)
    else:
        label_list = processor.get_labels() # ['O','B-LOC','I-LOC']
        if type_list[0] == data_type:
            examples = processor.get_train_examples(args.data_dir) # len(examples)=13637
        ...
        # 输入的是example[i]有[guid,text,label]三个字段
        # tokenizer是BertTokenizer， args.max_seq_length = 128  label_list是3个实体
    ...
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long) # [13637,128]
    all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long) # [13637,128]
    all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long) # [13637,128]
    all_label = torch.tensor([f.label for f in features], dtype=torch.long) # [13637,128]
...
def main():
    ...
    model = BertCrf(
        ...
        model_name=args.pre_train_model,
